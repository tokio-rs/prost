...
pub fn encode_message<M, B>(tag: u32, msg: &M, buf: &mut B)
where M: Message,
      B: BufMut {
    encode_key(tag, WireType::LengthDelimited, buf);
...
macro_rules! message_map {
    ($key_ty:ty,
     ...
     $key_encoded_len:ident) => (
         ...
         #[inline]
         pub fn $encode<M, B>(tag: u32, values: &HashMap<$key_ty, M>, buf: &mut B)
         where M: Message, B: BufMut {
            for (key, val) in values {
                let skip_key = key == &<$key_ty as Default>::default();
                let skip_val = val == &M::default();

                let len = (if skip_key { 0 } else { $key_encoded_len(1, key) }) +
                          (if skip_val { 0 } else { encoded_len_message(2, val) });
                ...
                if !skip_val {
                    encode_message(2, val, buf);
                }
            }
         }

         #[inline]
         pub fn $merge<M, B>(values: &mut HashMap<$key_ty, M>,
                             buf: &mut Take<B>) -> Result<()>
        where M: Message,
              B: Buf {
            let len = decode_varint(buf)?;
            if len > buf.remaining() as u64 {
            ...
            let mut key = Default::default();
            let mut val = M::default();

            while buf.has_remaining() {
                let (tag, wire_type) = decode_key(buf)?;
                match tag {
                    1 => $key_merge(wire_type, &mut key, buf)?,
                    2 => merge_message(&mut val, buf)?,
                    _ => (),
                }
            }
...
message_map!(u32, encode_map_uint32_message, merge_map_uint32_message, encoded_len_map_uint32_message, encode_uint32, merge_uint32, encoded_len_uint32);
message_map!(u64, encode_map_uint64_message, merge_map_uint64_message, encoded_len_map_uint64_message, encode_uint64, merge_uint64, encoded_len_uint64);
message_map!(i32, encode_map_sint32_message, merge_map_sint32_message, encoded_len_map_sint32_message, encode_sint32, merge_sint32, encoded_len_sint32);
message_map!(i64, encode_map_sint64_message, merge_map_sint64_message, encoded_len_map_sint64_message, encode_sint64, merge_sint64, encoded_len_sint64);
message_map!(u32, encode_map_fixed32_message, merge_map_fixed32_message, encoded_len_map_fixed32_message, encode_fixed32, merge_fixed32, encoded_len_fixed32);
message_map!(u64, encode_map_fixed64_message, merge_map_fixed64_message, encoded_len_map_fixed64_message, encode_fixed64, merge_fixed64, encoded_len_fixed64);
message_map!(i32, encode_map_sfixed32_message, merge_map_sfixed32_message, encoded_len_map_sfixed32_message, encode_sfixed32, merge_sfixed32, encoded_len_sfixed32);
