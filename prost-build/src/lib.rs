...
pub struct Config {
    ...
    post_compile_hook: Option<Box<dyn FnMut(&mut Config, &FileDescriptorSet)>>,
...
impl Config {
    ...
    /// Register a callback to configure additional options based on the `FileDescriptorSet`
    /// generated by `protoc`.
    ///
    /// # Examples
    ///
    /// This may be used to apply attributes with more complex logic than methods like `type_attribute`.
    ///
    /// ```rust
    /// # let mut config = prost_build::Config::new();
    /// // Nothing around uses floats, so we can derive real `Eq` in addition to `PartialEq`.
    /// config.post_compile_hook(|config, file_set| {
    ///     for file in &file_set.file {
    ///         for message in &file.message_type {
    ///             // Apply an attribute to top-level message types only.
    ...
    /// });
    /// ```
    pub fn post_compile_hook<F>(&mut self, hook: F) -> &mut Self
    where
        F: FnMut(&mut Config, &FileDescriptorSet) + 'static,
    {
        self.post_compile_hook = Some(Box::new(hook));
        self
    }
    ...
    pub fn compile_protos(
        ...
        if let Some(mut post_compile_hook) = self.post_compile_hook.take() {
            post_compile_hook(self, &file_descriptor_set);
            self.post_compile_hook = Some(post_compile_hook);
        }
