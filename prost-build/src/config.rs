...
use prost_types::{Edition, FileDescriptorProto, FileDescriptorSet};
...
impl Config {
    ...
    /// Processes a set of modules and file descriptors, returning a
    /// [`CodeGeneratorResponse`] suitable for emission by a `protoc` plugin.
    pub fn generate_code_generator_response(
        &mut self,
        requests: Vec<(Module, FileDescriptorProto)>,
        ...
        let mut files: Vec<_> = modules
            ...
            .map(|(module, content)| code_generator_response::File {
                name: Some(module.to_file_name_or(&self.default_package_filename)),
        ...
        Ok(CodeGeneratorResponse {
            ...
            minimum_edition: Some(Edition::Proto2 as i32),
            maximum_edition: Some(Edition::Edition2023 as i32),
            file: files,
        })
    }
