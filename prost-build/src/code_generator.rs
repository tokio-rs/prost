...
impl<'b> CodeGenerator<'_, 'b> {
    ...
    fn append_field(&mut self, fq_message_name: &str, field: &Field) {
        let type_ = field.descriptor.r#type().unwrap_or_default();
        let repeated = field.descriptor.label().unwrap_or_default() == Label::Repeated;
        ...
        match field.descriptor.label().unwrap_or_default() {
            Label::Optional => {
                if optional {
    ...
    fn resolve_type(&self, field: &FieldDescriptorProto, fq_message_name: &str) -> String {
        match field.r#type().unwrap_or_default() {
    ...
    fn optional(&self, field: &FieldDescriptorProto) -> bool {
        ...
        if field.label().unwrap_or_default() != Label::Optional {
            return false;
        }
