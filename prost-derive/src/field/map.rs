...
impl Field {
    ...
    pub fn debug(&self, wrapper_name: TokenStream, prost_path: &TokenStream) -> TokenStream {
        ...
        let key_wrapper = fake_scalar(self.key_ty.clone()).debug(quote!(KeyWrapper), prost_path);
...
impl ValueTy {
    ...
    fn debug(&self, prost_path: &TokenStream) -> TokenStream {
        match self {
            ValueTy::Scalar(ty) => fake_scalar(ty.clone()).debug(quote!(ValueWrapper), prost_path),
            ValueTy::Message => quote!(
